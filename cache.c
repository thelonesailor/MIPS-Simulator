...
void set_default()
{
	...
	cache_block_size[u][0] = DEFAULT_CACHE_BLOCK_SIZE;
	cache_block_size[u][1] = DEFAULT_CACHE_BLOCK_SIZE;
	
	words_per_block[0] = DEFAULT_CACHE_BLOCK_SIZE / WORD_SIZE;
	words_per_block[1] = DEFAULT_CACHE_BLOCK_SIZE / WORD_SIZE;

	cache_assoc[0] = DEFAULT_CACHE_ASSOC;
	cache_assoc[1] = DEFAULT_CACHE_ASSOC;

	cache_writeback[0] = DEFAULT_CACHE_WRITEBACK;
	cache_writeback[1] = DEFAULT_CACHE_WRITEBACK;

	cache_writealloc[0] = DEFAULT_CACHE_WRITEALLOC;
	cache_writealloc[1] = DEFAULT_CACHE_WRITEALLOC;
} 
...
void init_vars()
{
	...
	int sizei,sized;
	if(cache_split==0)
	{sizei=sized=cache_usize+2*cache_block_size[u]*cache_assoc+10;}
	else if(cache_split==1)
	{sizei=cache_isize+2*cache_block_size[u]*cache_assoc+10;
	sized=cache_dsize+2*cache_block_size[u]*cache_assoc+10;}
	else
...
void init_cache()
{
	...
	if(cache_split==0 && cache_usize % (cache_assoc*cache_block_size[u])!=0)
	{
		fprintf(stderr,"Error - cache_usize is not divisible by assocciativity*block_size\n");
		fflush(stderr);
	}
	else if(cache_split==1 && cache_isize % (cache_assoc*cache_block_size[u])!=0)
	{
		fprintf(stderr,"Error - cache_isize is not divisible by assocciativity*block_size\n");
		fflush(stderr);
	}
	else if(cache_split==1 && cache_usize % (cache_assoc*cache_block_size[u])!=0)
	{
		fprintf(stderr,"Error - cache_usize is not divisible by assocciativity*block_size\n");
		fflush(stderr);
	}
	...
	{
		blockbits[0]=LOG2(cache_block_size[u]);

		setsize[0]=cache_assoc*cache_block_size[u];
	...
	else if(cache_split==1)//split cache
	{
		...
		//for D	
		blockbits[1]=LOG2(cache_block_size[u]);

		setsize[1]=cache_assoc*cache_block_size[u];
	...
	//	words_per_block = cache_block_size[u] / 4;
...
{
	// v[i] and tag[i] are valid only for i which are multiples of cache_block_size[u] 
	int hit=0;
	for(int i=(indx+1)*setsize[u] ; i<end ; i+=cache_block_size[u])
	{
  ...
  if(hit==0)//miss
  {
    ...
    //write to main memory the block from [mini] to [mini+cache_block_size[u]-1]        
    {
	...
	/*	//block write to MM from here 
		...
		for(unsigned i=mini,j=0 ; i<(mini+cache_block_size[u]) ; ++i,++j)
		{
			//write_data=c[u][i];
			//write_to_MM(pre|j);
		} 
...
void WB_WNA(unsigned addr,unsigned access_type,int u)
{
  ...
  if(hit==0)//miss
  {
	...
	*/
    ...
    {
      for(int i=mini ; i<(mini+cache_block_size[u]) ; ++i)
      {c[u][i]=1;/*=from memory*/
...
void flush(int u)
{
  unsigned i=0,end=(numsets[u])*setsize[u];
    ...
    for(i=0 ; i<end ; i+=cache_block_size[u])
    {
      if(dirty[u][i]==true)
      {
        unsigned j=i;
        for(j=i ; j<(i+cache_block_size[u]) ; ++j)
        {
          //mem[...]=c[j]
        }
...
void dump_settings()
{
  ...
  if (cache_split) {
	...
	printf("  I-Associativity: \t%d\n", cache_assoc[0]);
	printf("  I-Block size: \t%d\n", cache_block_size[0]);
	printf("  I-Write policy: \t%s\n", 
	 cache_writeback[0] ? "WRITE BACK" : "WRITE THROUGH");
	printf("  I-Allocation policy: \t%s\n",
	 cache_writealloc[0] ? "WRITE ALLOCATE" : "WRITE NO ALLOCATE");

    printf("  D-cache size: \t%d\n", cache_dsize);
	printf("  D-Associativity: \t%d\n", cache_assoc[1]);
	printf("  D-Block size: \t%d\n", cache_block_size[1]);
	printf("  D-Write policy: \t%s\n", 
	 cache_writeback[1] ? "WRITE BACK" : "WRITE THROUGH");
	printf("  D-Allocation policy: \t%s\n",
	 cache_writealloc[1] ? "WRITE ALLOCATE" : "WRITE NO ALLOCATE");
